...
train_model= str(input('choose model to test'))
...
test_data="C:/Users/Maryem/Desktop/fer2013/test.csv"
...
def get_data(dataset):
    file_stream=file_io.FileIO(test_data,mode='r')
    data=pd.read_csv(file_stream)
    pixels=data["pixels"].tolist()
    images=np.empty((len(data),img_height,img_width,3))
    i=0
    for pixel in pixels:
        single_image=[float(p) for p in pixel.split(" ")]
...
images, labels = get_data(test_data)
